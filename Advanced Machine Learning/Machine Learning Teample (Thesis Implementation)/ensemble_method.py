...
base1 = LogisticRegression(solver='lbfgs', multi_class='auto')
base2 = MultinomialNB()
base3 = CalibratedClassifierCV(RidgeClassifier(solver='sparse_cg'), cv=5)
base4 = SGDClassifier(loss='log',random_state=0)
base5 = RandomForestClassifier(n_estimators=10, max_depth=2)
...
for clf, label in zip([bg_clf1, bg_clf2, bg_clf3, bg_clf4, bg_clf5], ['Logistic Regression', 'Multinomial NB', 'RidgeClassifier', 'SGDClassifier','Randomforest']):
...
clf5 = RandomForestClassifier(n_estimators=10, max_depth=2)

eclf = VotingClassifier(estimators=[('lr', clf1), ('mnb', clf2), ('rc', clf3), ('SGD', clf4), ("RF", clf5)], voting='hard')
...
new_proba_sum = new_proba_SGD + new_proba_LR + new_proba_MNB + new_proba_RC + new_proba_RF
...
data = {"Ensemble_accuracy" : ["Bagging(LR)","Bagging(MNB)","Bagging(RC)","Bagging(SGD)","Bagging(RF)",
                                "Simple_Voting", "BMA"],
